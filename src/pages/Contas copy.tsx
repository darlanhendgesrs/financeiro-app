...
export default function Contas() {
  ...
  const fetchBills = async (limite = limit) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      const inserts = months.map((month) => {
        const dueDate = month.date(recurringDay).format('YYYY-MM-DD')
        return {
          description,
          amount,
          vat: vatAmount,
  ...
  const markAsPaid = async (bill: Bill) => {
    ...
    if (!insertError) {
      ...
      if (!deleteError) fetchBills(limit)
    }
  }

  const deleteBill = async (id: string) => {
    const { error } = await supabase.from('bills').delete().eq('id', id)
    if (!error) fetchBills(limit)
  }
    ...
    <div className="max-w-2xl mx-auto p-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-2 mb-6">
        ...
        <button type="submit" className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
          {isRecurring ? 'Criar recorrência' : 'Adicionar'}
        </button>
      </form>
        ...
        {bills.map((bill) => (
          <li key={bill.id} className="border p-3 rounded bg-white flex flex-col">
            <div className="flex justify-between items-center">
              <span className={`font-semibold ${getStatusColor(bill)}`}>
